<div class="wrap">
  ...
  <a class="add-new-h2" target="_blank" href="<?php echo esc_url("https://www.youtube.com/watch?v=hUCeDRk5LhI"); ?>"><?php _e('Watch Tutorial', 'collectchat'); ?></a>
  </div>
      ...
      <div class="postbox">
        <div class="inside">
          <form name="dofollow" action="options.php" method="post">
            ...
            <p><b>3.</b> <?php _e('Copy the code snippet from Dashboard > Publish and paste it here', 'collectchat'); ?></p>
            </div>
            <h3 class="cc-labels" for="script"><?php _e('Chatbot Snippet:', 'collectchat'); ?></h3>
            <textarea id="collectchat-plugin-snippet" style="width:100%;" rows="5" cols="50" id="script" name="collectchat-plugin-settings[script]"><?php echo esc_html($script); ?></textarea>

            <p>
              <h3>Show Above Chatbot On: </h3>
              <input type="radio" name="collectchat-plugin-settings[showOn]" value="all" id="all" <?php checked('all', $showOn); ?>> <label class="collectchat-plugin-label" for="all"><?php _e('Everywhere', 'collectchat'); ?> </label> 
              <input type="radio" name="collectchat-plugin-settings[showOn]" value="home" id="home" <?php checked('home', $showOn); ?>> <label class="collectchat-plugin-label" for="home"><?php _e('Homepage Only', 'collectchat'); ?> </label> 
              <input type="radio" name="collectchat-plugin-settings[showOn]" value="nothome" id="nothome" <?php checked('nothome', $showOn); ?>> <label class="collectchat-plugin-label" for="nothome"><?php _e('Everywhere except Home', 'collectchat'); ?> </label>
              <input type="radio" name="collectchat-plugin-settings[showOn]" value="none" id="none" <?php checked('none', $showOn); ?>> <label class="collectchat-plugin-label" for="none"><?php _e('Nowhere', 'collectchat'); ?> </label>
            </p>

            <p class="submit">
              <input class="button button-primary" type="submit" name="Submit" value="<?php _e('Save', 'collectchat'); ?>"  style="padding: 0px 30px;font-size:15px;background-color: #2c6ac3;border-color: #2c6ac3;"/>
            </p>
